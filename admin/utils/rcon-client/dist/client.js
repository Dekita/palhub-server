...
class RconClient extends node_events_1.EventEmitter {
    ...
    static async connect(options) {
        const socket = (0, node_net_1.createConnection)(options);
    ...
    async auth(password) {
        ...
        try {
            ...
            if (res.type != 2) {
                throw new Error(`Unexpected response type (expected 2, got ${res.type})`);
            }
            this.#authed = true;
        }
        catch (e) {
    ...
    async cmd(cmd) {
        ...
        try {
            ...
            if (res.type != 0) {
                throw new Error(`Unexpected response type (expected 0, got ${res.type})`);
            }
    ...
    async #send(id, type, message) {
        ...
        await new Promise((resolve, reject) => this.socket.write(buf, (err) => {
            if (err)
                reject(err);
            else
                resolve();
    ...
    #nextReqId() {
        const id = this.#reqId;
        this.#reqId = (this.#reqId + 1) % 2147483648;
        return id;
    }
}
