...
class DekCheckBox extends EventTarget {
    ...
    updateValue() {
        ...
        if (!this.ismulti && !this.idcheck) {
            const checked_val =
                this.checkul.getElementsByClassName('checked')[0];
    ...
    getLiNumVal() {
        ...
        for (let i = this.options.length - 1; i >= 0; i--) {
            const tempval = this.idcheck ? i : this.getLiTexVal(i);
            if (this.options[i].classList.contains('checked')) {
                this.ismulti ? value.push(tempval) : (value = tempval);
            }
        }
        return this.ismulti ? value.reverse().join(',') : value;
    }
    getLiTexVal(i) {
        const span = this.options[i].getElementsByTagName('span');
        return span ? span[0].innerText.trim() : 'error';
    }
    setActive(index) {
        this.updateSyblings();
        this.updateValue();
...
class DekSelect extends EventTarget {
    ...
    get visible() {
        return this._ul?.classList.contains('d-block');
    }
    get value() {
    ...
    initialize(element) {
        ...
        this._element.parentElement.insertBefore(
            this._a,
    ...
    _initListener() {
        this._a.addEventListener('click', async (event) => {
            ...
            if (event.target.nodeName === 'LI')
                this.set(event.target.innerText);
        });
    }
    set(value) {
...
function loadCustomthemeFromStorage(css_string) {
    ...
    const css_properties =
        css_string || localStorage.getItem('dek-theme').split(';');
...
document.addEventListener('DOMContentLoaded', async (event) => {
    ...
    for (const element of [...checkers])
        new DekCheckBox(element, (handler) => {});
});
